...
try:
    from stargan.model import Generator, AvgBlurGenerator
...
class Solver_jpeg(object):
    """Solver for training and testing StarGAN."""

    def __init__(self, celeba_loader, rafd_loader, config, compression_model, config_attack):
        ...
        # load DiffJPEG
        self.compress_network, self.decompress_network = compression_model
        self.compress_network.to(self.device)
        self.decompress_network.to(self.device)

        self.epsilon = config_attack.epsilon
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if (i + 1) % self.model_save_step == 0:
                ...
                print('Saved model checkpoint into {}...'.format(self.model_save_dir))
    ...
    def test_universal_model_level(self, i, x_real, c_org, perturb, args):
        ...
        with torch.no_grad():
            ...
            c_trg_list = self.create_labels(c_org, self.c_dim, self.dataset, self.selected_attrs)
            # Translated images.
            x = x_real.clone().to(self.device)
            x = (x + 1) / 2
            y_, cb_, cr_ = self.compress_network(x)                                     
            perturb = torch.clamp(perturb, -self.epsilon, self.epsilon)

            x_adv = self.decompress_network(y_ + perturb, cb_, cr_)
            x_adv = torch.clamp(x_adv * 2 - 1, -1, 1).to(self.device)
            # x_real = self.decompress_network(y_, cb_, cr_)
            # x_real = (x_real * 2 - 1).to(self.device)
        # x_adv = torch.clamp(x_real + perturb, -1, 1)
        ...
        return x_noattack_list, x_fake_list[2:], x_adv

    def test_universal_model_jpeg_attack(self, i, x_real, c_org, pgd_attack):
        """try JPEG-Robust"""
        ...
        for idx, c_trg in enumerate(c_trg_list):
            ...
            with torch.no_grad():
                x_real_mod = x_real
                # x_real_mod = self.blur_tensor(x_real_mod) # use blur
                gen_noattack, gen_noattack_feats = self.G(x_real_mod, c_trg)  # (-1, 1)

            # Attacks
            x_adv, perturb = pgd_attack.universal_perturb_DiffJPEG(x_real, gen_noattack, c_trg, self.G,
                                                                   self.compress_network, self.decompress_network)
